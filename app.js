...
app.get('/get_photos', function (req, res) {
    ...
    fs.readdir(filesPath, function (err, files) {
        ...
        files.forEach(function (file) {
            ...
            // get the file type and remove last element
            let type = fileArr.pop();
            ...
            fileArr = fileArr.join('.').split('-');

            // remove the first element (the date)
            fileArr.shift();

            // there might have been '-' in the original name
            let name = fileArr.join('-');
